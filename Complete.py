...
square = [x ** 2 for x in range(left_border, right_border + 1)]
cube = [x ** 3 for x in range(left_border, right_border + 1)]

print(f"Список кубов чисел в диапозоне от {left_border} до {right_border} {cube}")
print(f"Список квадратов чисел в диапозоне от {left_border} до {right_border} {square}")
...
double = [x * 2 for x in line]
plus_symb = [x + symb for x in double]

print(f"Список удвоенных символов: {double}")
print(f"Склейка с дополнительным символом: {plus_symb}")

# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#расчет стоимости повышения цен 
